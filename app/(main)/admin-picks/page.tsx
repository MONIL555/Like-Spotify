...
import { Play, Shuffle, Clock, Heart } from 'lucide-react';
...
export default function AdminPicksPage() {
  ...
  const tracks = data?.madeForYou?.map((item: any) => item.data) || [];
  ...
  if (error) {
    return (
      <div className="clay-card p-12 text-center mt-12">
        <h3 className="text-xl font-bold text-destructive">Error loading Admin Picks.</h3>
      </div>
    );
  }
  ...
  return (
    <div className="py-6 flex flex-col gap-8 animate-fade-in">
      ...
      {/* Header */}
      <div className="flex flex-row items-center md:items-end gap-4 md:gap-8">
        <div className="relative h-28 w-28 md:h-56 md:w-56 shrink-0 clay-panel overflow-hidden bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center">
           {tracks.length > 0 && (
             <Image src={tracks[0].thumbnails?.high || tracks[0].thumbnails?.default || ''} alt="Admin Picks" fill className="object-cover opacity-80" />
           )}
           <div className="absolute inset-0 bg-black/40 flex items-center justify-center backdrop-blur-sm">
             <Heart className="h-12 w-12 md:h-20 md:w-20 text-white fill-white drop-shadow-md" />
           </div>
        </div>
        <div className="flex flex-col flex-1 pb-1 md:pb-2">
          ...
          <h1 className="text-2xl md:text-5xl lg:text-6xl font-bold text-foreground mb-2 md:mb-4 line-clamp-2">Admin Picks</h1>
      ...
      <div>
        ...
        {tracks.length === 0 ? (
          <div className="clay-card p-12 text-center mt-4">
            <h3 className="text-xl font-bold text-muted-foreground">No Admin Picks available yet.</h3>
          </div>
        ) : (
